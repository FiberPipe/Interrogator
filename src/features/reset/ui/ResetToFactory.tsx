...
export const ResetToFactoryCard = ({ onReset }: ResetToFactoryCardProps) => {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      addSuccessToaster(t('settings.reset.success'), t('settings.reset.successDescription'));
      ...
      <Card className="border-2 border-danger-200 dark:border-danger-900">
        <CardHeader className="flex gap-3">
          ...
          <div className="flex flex-col">
            <h4 className="text-lg font-semibold text-danger">{t('settings.reset.title')}</h4>
            <p className="text-sm text-default-500">{t('settings.reset.description')}</p>
          </div>
        </CardHeader>
