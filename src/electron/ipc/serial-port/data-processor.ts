...
import { sensorDataService } from '../database/sensor-data.service';
...
export class SerialDataProcessor {
  ...
  private saveInterval = 1000; // Сохраняем не чаще раза в секунду для оптимизации
  ...
  async processData(dataString: string) {
    try {
      ...
      // Отправляем данные на клиент (быстро, без блокировки)
      this.sendToClient(parsed);

      // Сохраняем в БД (асинхронно, не блокируя основной поток)
  ...
  async endSession() {
    if (this.sessionId) {
      ...
      console.log(`[DataProcessor ${this.port}] Session ended:`, this.sessionId, 'Records:', this.recordCount);
    }
  }
